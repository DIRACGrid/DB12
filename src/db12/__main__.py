...
def single_dirac_benchmark_cli(args):
    ...
    result = single_dirac_benchmark(args.iterations)["NORM"]
    if args.json:
...
"""
    ...
    parent_parser.add_argument("--iterations", nargs='?', type=int, help="number of iterations to perform", default=iterations)
    ...
    parser_single = subparsers.add_parser('single', help="get normalized power of one CPU", parents=[parent_parser])
    parser_single.set_defaults(func=single_dirac_benchmark_cli)

    parser_wholenode = subparsers.add_parser('wholenode', help="Run as many copies as needed to occupy the whole machine", parents=[parent_parser])
    parser_wholenode.add_argument("--extra-iteration", help="whether an extra iteration is needed", action='store_true')
    parser_wholenode.set_defaults(func=wholenode_dirac_benchmark_cli)

    parser_jobslot = subparsers.add_parser('jobslot', help="Run as many copies as needed to occupy the job slot", parents=[parent_parser])
    parser_jobslot.add_argument("--extra-iteration", help="whether an extra iteration is needed", action='store_true')

    parser_jobslot.set_defaults(func=jobslot_dirac_benchmark_cli)

    parser_multiple = subparsers.add_parser('multiple', help="Run multiple copies in parallel", parents=[parent_parser])
    
    parser_multiple.add_argument("copies", type=int, help="number of copies")
    parser_multiple.add_argument("--extra-iteration", help="whether an extra iteration is needed", action='store_true')
